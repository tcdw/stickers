...
export default function Navigation({ avatar }: Props) {
  ...
  const menuItemTimer = useRef<number | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      if (menuItemTimer.current !== null) {
        window.clearTimeout(menuItemTimer.current);
      }
    };
  }, []);
  ...
  function handleMobileMenuToggle(to?: boolean) {
    ...
    if (nextState) {
      ...
      menuItemTimer.current = window.setTimeout(() => {
  ...
  return (
    <nav className="fixed w-full top-0 z-40">
      <div
        id="navBar"
        className={
          "absolute -translate-x-2/4 left-2/4 h-(--navBar-height) md:h-[inherit] backdrop-blur-xl bg-white/80 shadow-xl shadow-black/5 [transition:top_150ms,height_400ms_cubic-bezier(.47,1.64,.41,.8)] overflow-clip rounded-[2.25rem] top-4 w-[calc(100dvw-2rem)] md:w-max"
        }
        style={
          {
